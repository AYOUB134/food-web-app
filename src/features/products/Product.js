...
const ProductComponent = () => {
  ...
  return (
    <div className="relative">
      <div className="fixed top-8 left-0 right-0 bg-white z-40 p-4 shadow-md"> {/* Fixed search and categories */}
      ...
      <div className="pt-32"> {/* Added padding top to account for the fixed search and categories */}
        {showAllCategories && (
        ...
        <div>
          {filteredProducts.map(product => {
            ...
            return (
              <div key={product.id} className={`flex justify-between items-center mb-4 p-4 border-b ${isFirstProduct ? 'relative w-90' : ''}`}>
                <div className={`flex items-center ${isFirstProduct ? 'relative w-90' : ''}`}>
                  <img src="https://api.chowbus.com/api/v2/image_proxy/blobs/eyJfcmFpbHMiOnsibWVzc2FnZSI6IkJBaHBBNGcvUWc9PSIsImV4cCI6bnVsbCwicHVyIjoiYmxvYl9pZCJ9fQ==--31c201caecc355c8996105144613917432bddc54/%E5%9C%9F%E8%B1%86%E4%B8%9D%E7%89%9B%E8%82%89%E7%9B%96%E6%B5%87%E9%A5%AD.jpeg" alt={product.title} className={`rounded mr-4 border border-gray-200 ${isFirstProduct ? 'w-90 h-auto' : 'w-16 h-16'}`} />
                  {isFirstProduct && (
                    <div className="absolute top-0 left-0 m-2">
                      <h6 className="text-sm font-bold truncate" style={{ fontSize: '12px', color: 'white' }}>{product.title}</h6>
                      <p className="text-gray-200" style={{ fontSize: '12px', color: 'white' }}>${product.price}</p>
                    </div>
                  )}
                  {!isFirstProduct && (
                    <div>
                ...
                {isFirstProduct && (
                  <button
                    className="absolute bottom-2 right-2 text-white bg-red-500 rounded-full w-7 h-7 flex items-center justify-center"
                    onClick={() => handleAddToCart(product)}
                    style={{ backgroundColor: '#e00051' }}
                  >
                    +
